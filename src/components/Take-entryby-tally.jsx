...
const TakeEntryTallyPage = () => {
  ...
  const submitFormData = async (rowId) => {
    ...
    try {
      ...
      setSubmittedRows(prev => new Set([...prev, `tally_${rowId}`]));
    ...
    return (
      <div className="p-6 bg-gray-50 min-h-screen">
        <div className="flex items-center justify-center h-64">
          ...
          <span className="ml-2 text-lg text-gray-600">Loading tally data...</span>
        </div>
      </div>
    );
  }
      ...
      <div className="bg-white rounded-lg shadow border mb-4">
        <div className="px-4 py-3 border-b">
          <div className="flex items-center justify-between">
            <div>
              <h3 className="text-base font-medium text-gray-900">Take Entry By Tally</h3>
              ...
              {accountsData.length === 0 ? (
                <tr>
                  <td colSpan={9} className="px-4 py-8 text-center text-gray-500">
                    No records available for tally entry
                  </td>
                </tr>
              ) : (
